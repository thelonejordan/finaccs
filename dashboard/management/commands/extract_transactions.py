"""
Extract transactions from bank statement files to standardized CSV blobs.

This is the first phase of the two-phase workflow:
1. Extract: Original file blob → Standardized CSV blob (this command)
2. Load: CSV blob → Transaction records (load_transactions command)
"""
import fnmatch
import gzip
import hashlib
...
from bank_accs.models import SourceFile, ExtractionPipeline, ExtractedCSV
from bank_accs.extractors import EXTRACTORS, get_extractor, detect_extractor
...
class Command(BaseCommand):
    help = 'Extract transactions from bank statement files to standardized CSV blobs'

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            help='Path to a specific bank statement file to extract',
        )
        parser.add_argument(
            '--all',
            action='store_true',
            help='Extract all files from bank_accs/data/',
        )
        parser.add_argument(
            '--force',
            action='store_true',
        ...
        parser.add_argument(
            '--password',
            type=str,
            help='Password for encrypted files (overrides pipeline password)',
        )
        parser.add_argument(
            '--store-original',
            action='store_true',
    ...
    def extract_all_files(self):
        """Extract all files from bank_accs/data/"""
    ...
    def extract_file(self, file_path):
        ...
        self.stdout.write(f'Extracting transactions from {file_path}')
        ...
        # Extract to CSV
        try:
            csv_string = extractor_func(file_path, password=password)
        ...
        # Create new ExtractedCSV record
        ...
        self.stdout.write(self.style.SUCCESS(
            f'  Created ExtractedCSV #{extracted_csv.id} with {row_count} rows'
        ))
        return row_count

...
def _build_payment_link_maps(bank_rt_ids, cc_rt_ids):
    """Build payment link maps for bank and CC resolved transaction IDs."""
    from links.models import CreditCardPaymentLink

    cc_payment_links = CreditCardPaymentLink.objects.filter(
        is_active=True
    ).select_related('bank_resolved_transaction', 'cc_resolved_transaction')
    ...
    return bank_rt_cc_link_map, cc_rt_bank_link_map
...
def get_entity_transactions(entity):
    ...
    from links.models import Breakdown

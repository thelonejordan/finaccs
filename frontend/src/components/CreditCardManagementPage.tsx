...
// Main Credit Card Management Page
export function CreditCardManagementPage() {
  ...
  useEffect(() => {
    document.title = "Credit Card Management | FinAccs"
  }, [])
  ...
  const handleCardSave = (card: CreditCard) => {
    setCards((prev) => {
  ...
  const handleSourceFileUpdated = (sourceFile: CreditCardSourceFile) => {
    setSourceFiles((prev) =>
      prev.map((sf) => (sf.id === sourceFile.id ? sourceFile : sf))
    )
  }

  const handleCreateCard = (filename: string) => {
    setAddSourceFile(filename)
  }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <header className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">
            Manage your credit cards and link source files
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CreditCardDataSources
            ...
            onSourceFileUpdated={handleSourceFileUpdated}
          />
        </div>
      </main>
    </div>

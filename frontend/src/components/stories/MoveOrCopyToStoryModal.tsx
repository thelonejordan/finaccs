...
import { XIcon, PlusIcon, MoveIcon, CopyIcon, BookOpenIcon, UsersIcon, UserIcon, BuildingIcon, SearchIcon } from "lucide-react"
import {
  fetchStories,
  fetchEntities,
...
export function MoveOrCopyToStoryModal({
  ...
  // Create form state for stories
  const [newName, setNewName] = useState("")
  const [newIcon, setNewIcon] = useState("💰")
  const [isCreating, setIsCreating] = useState(false)

  // Create form state for entities
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setStories(storiesData.stories.filter(s => s.story_id !== currentStoryId))
  ...
  const handleMoveOrCopyToEntity = async (targetEntityId: string) => {
    ...
    try {
      ...
      await addTransactionsToEntity(targetEntityId, selectedTransactions)
  ...
  const handleCreateAndMoveOrCopy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : `Failed to create ${activeTab === "stories" ? "story" : "entity"}`)
    } finally {
      setIsCreating(false)
    }
  }

  const currentEmojiOptions = activeTab === "stories" ? STORY_EMOJI_OPTIONS : ENTITY_EMOJI_OPTIONS
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-card rounded-lg shadow-xl p-6 w-full max-w-md max-h-[80vh] overflow-hidden z-50 flex flex-col">
          <Dialog.Title className="text-lg font-semibold text-foreground flex items-center gap-2">
            ...
            {actionLabel} to Story or Entity
          </Dialog.Title>
          <Dialog.Description className="mt-1 text-sm text-muted-foreground">
            {actionLabel} {selectedTransactions.length} transaction{selectedTransactions.length !== 1 ? "s" : ""} from current story.
              ...
              <form onSubmit={handleCreateAndMoveOrCopy} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={activeTab === "stories" ? "e.g., Japan Trip 2024" : (newEntityType === "person" ? "e.g., John Doe" : "e.g., Amazon")}

...
import {
  ...
  compareStories,
  type Story,
  type StoryComparisonResult,
  type StoryTransaction,
} from "@/lib/api"

interface CompareStoriesModalProps {
  open: boolean
...
function TransactionTable({ transactions }: { transactions: StoryTransaction[] }) {
...
export function CompareStoriesModal({ open, onOpenChange }: CompareStoriesModalProps) {
  ...
  const [result, setResult] = useState<StoryComparisonResult | null>(null)
  ...
  const handleCompare = async () => {
    ...
    try {
      const data = await compareStories(Array.from(selectedIds))
      setResult(data)
      setView("results")
      setActiveTab("common")
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to compare stories")
    } finally {
      setComparing(false)
    }
  }
  ...
  const getTabTransactions = (): StoryTransaction[] => {
    if (!result) return []
    if (activeTab === "common") {
            ...
            {view === "selection" ? (
              <>
                <Dialog.Description className="text-sm text-muted-foreground mb-4">
                  Select 2 or more stories to compare their transactions.
                </Dialog.Description>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-12 text-muted-foreground">
                    You need at least 2 stories to compare.
                  </div>
                ) : (
                    ...
                    {stories.map((story) => (
                      <label
                        ...
                        className={`flex items-center gap-4 p-4 rounded-lg border cursor-pointer transition-colors ${
                          selectedIds.has(story.story_id)
                      ...
                      >
                        <input
                          ...
                          onChange={() => toggleSelection(story.story_id)}
                          className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                        />
                        <span className="text-2xl">{story.icon}</span>
                        <div className="flex-1 min-w-0">
                          <div className="font-medium truncate">{story.name}</div>
                          <div className="flex items-center gap-4 text-xs text-muted-foreground mt-0.5">
                            <span className="flex items-center gap-1">
                              <HashIcon className="h-3 w-3" />
                              {story.transaction_count} txns
                            </span>
                            <span className="flex items-center gap-1">
              ...
              <>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 mb-4 border-b border-border">
                  ...
                  {result.stories.map((story) => (
                    <button
                      key={story.story_id}
                      onClick={() => setActiveTab(story.story_id)}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-colors flex items-center gap-1.5 ${
                        activeTab === story.story_id
                          ? "bg-primary text-primary-foreground"
                          : "bg-muted hover:bg-accent"
                      }`}
                    >
                      <span>{story.icon}</span>
                      {story.name} Only ({result.unique_transactions[story.story_id]?.length || 0})
                    </button>
                  ))}
                </div>
